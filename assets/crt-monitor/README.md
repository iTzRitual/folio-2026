...
`CRT_Screen` preserves its 0.352 × 0.264 m dimensions, positive-Z normals, UVs, and crown at Z=0. `CRT_Glass` supplies the peripheral contour. The portfolio hides these source meshes and uses their geometry in `CRTDisplay` to present the live desktop. `CRTMonitor` remains inside the same final surface group, preserving the reveal and desktop interaction coordinates.
