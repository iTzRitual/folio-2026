...
- `npm test` — deterministic architecture and rendering checks
...
An interactive portfolio (Next.js 16 App Router, React 19, TypeScript, Tailwind 4) whose Hero/Details UI is rendered inside a WebGL canvas via React Three Fiber while intentionally looking like a normal flat website. Its final scroll stage pulls the camera back to reveal that interface on an interactive CRT monitor in a 3D workstation.

## Architecture

Two parallel render paths live in `src/app/page.tsx` (a single client component):
...
2. **No-JS fallback** — `src/components/NoJs/` renders real HTML content; the JS app lives under `.js-only-app` (toggled via CSS in `globals.css`).

Key mechanisms that span multiple files:

- **Scroll → 3D animation**: `HeroTransitionProvider` exposes mutable refs (not state — updated per-frame without re-renders), read in `useFrame` by components inside the canvas. Scroll itself is smoothed by Lenis (`ReactLenis root`), mounted only after the loader finishes. See "Scroll timeline" below for what each ref means.
...
- **Workstation** (`src/components/WorkstationScene.tsx`): captures the portfolio into a virtual desktop, coordinates its app windows and interactions, and presents it through `CRTDisplay`, `CRTMonitor`, and `WorkstationEnvironment`.
...
The desktop page has four stages, and the scroll length is derived from content, not fixed:
...
4. **Workstation reveal** — pulls back from the portfolio surface to the interactive CRT and surrounding scene.

`HeroTransitionProvider` publishes four refs:
...
- `revealProgressRef` — 0→1 across the workstation camera pullback after the details overflow.
...
- **Adding a details section** starts in `src/data/detailsContent.ts`, whose typed keys, headings, and wide/narrow lines feed both layout modes. Add its `<DetailsSection>` presentation in `Details.tsx` and mirror it in `NoJsDetails.tsx`; the type-checked layout records and `npm test` catch missing geometry entries.
