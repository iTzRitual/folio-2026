  ...
  return <group name="PersonalBlockoutProps">
    ...
    <group name="MonsteraFloorPlant" position={position(w.floorPlantPosition)}>
      ...
      {Array.from({ length: 10 }, (_, i) => {
        const side = i % 2 ? -1 : 1;
        return <group key={i} position={[0, 0.36 + i * CONFIG.workstation.FLOOR_PLANT_LEAF_STEP, 0]} rotation={[0, side * 0.28, side * -CONFIG.workstation.FLOOR_PLANT_SPREAD]}>
          <Cylinder radius={0.004} height={0.21} position={[0, 0.105, 0]} color="#526049" />
          <mesh geometry={leaf} position={[0, 0.3, 0.015]} rotation={[0.15 + (i % 3) * 0.1, side * 0.25, 0]} scale={CONFIG.workstation.FLOOR_PLANT_LEAF_SCALE + (i % 3) * CONFIG.workstation.FLOOR_PLANT_LEAF_VARIATION} raycast={noRaycast}>
