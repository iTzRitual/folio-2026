...
Workstation reveal will reveal that the flat interface is displayed on a monitor inside a larger workstation scene. Maintaining a separate DOM mobile experience would prevent both viewport classes from sharing that reveal and would create two products that can drift independently.
...
- Header, hero, details, projects, case studies, themes, and the future Workstation reveal transition remain one product across viewport sizes.
